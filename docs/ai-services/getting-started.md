...
2. **Edit the `.env` file** with your API keys and configuration:
   ...
   # OpenAI Configuration (Required for text generation)
   OPENAI_API_KEY=your_openai_api_key_here

   # Google AI (Gemini) Configuration (Alternative to OpenAI)
   GOOGLE_API_KEY=your_google_api_key_here

   # Together.ai Configuration (Alternative AI provider)
   TOGETHER_API_KEY=your_together_api_key_here

   # OpenRouter.ai Configuration (Meta provider for multiple models)
...
3. **Important Notes on API Keys**:
   ...
   - You only need to configure **one** of the text generation providers (OpenAI, Google AI, or Together.ai)
   - The Image Generation service requires either Google's Vertex AI or OpenAI's DALL-E
