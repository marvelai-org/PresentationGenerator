...
class SlideGenerator:
    def __init__(self, args=None, vectorstore_class=Chroma, prompt=None, embedding_model=None, model=None, parser=None, verbose=False):
        ...
        if not google_api_key:
            ...
            logger.warning("Will try to proceed with generation without embeddings")
            # Don't raise error here, we'll handle it in generate_slides
            
        try:
            default_config = {
                "embedding_model": GoogleGenerativeAIEmbeddings(model='models/embedding-001', google_api_key=google_api_key) if google_api_key else None,
            ...
            logger.error(f"Error initializing SlideGenerator: {str(e)}")
            # Don't raise the error here, we'll try to proceed with basic functionality
    ...
    def generate_slides(self):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error generating slides with LLM: {str(e)}")
            ...
            logger.error(f"Error in slide generation workflow: {str(e)}")
            # Return a minimal valid response structure
            return {"slides": [
                {
                    "title": f"Error in slide generation workflow: {str(e)}",
                    "template": "titleAndBody",
                    "content": "Please check the API server logs for more details."
                }
            ]}
