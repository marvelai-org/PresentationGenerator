...
const createMockClient = () => {
  ...
  console.warn('🛑 Using mock Supabase client - database and authentication functionality will be limited');
  ...
  return {
    ...
    auth: {
      ...
      signInWithPassword: async () => ({
        data: null,
        error: { message: "Mock client - authentication disabled in this environment" }
      }),
      signInWithOtp: async () => ({
        data: null,
        error: null
    ...
    from: (table: string) => {
      ...
      return {
        ...
        select: (columns?: string | string[]) => {
          ...
          if (columns) {
            const columnList = typeof columns === 'string' 
        ...
        insert: (data: any | any[]) => {
          ...
          enhanced.select = () => createFilterBuilder(table, result.data ? [].concat(result.data) : []);
        ...
        rpc: (fn: string, params: any) => {
          ...
          return Promise.resolve({ data: null, error: new Error('RPC functions not implemented in mock client') });
        }
      };
    },
      ...
      from: (bucket: string) => {
        ...
        return {
          ...
          getPublicUrl: (path: string) => {
            ...
            return { publicUrl: `https://mock-storage.example.com/${bucket}/${path}` };
          },
          ...
          remove: async (paths: string | string[]) => {
            ...
            return { data: { count: Array.isArray(paths) ? paths.length : 1 }, error: null };
          }
        };
      }
    },
...
export function createClientComponentClient(options?: any) {
  console.warn('⚠️ createClientComponentClient is deprecated, please use createClientSupabaseClient instead');
