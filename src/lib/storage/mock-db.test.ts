...
async function mockDatabaseDemo() {
  ...
  const { data: allPresentations } = await supabase.from('presentations').select();
  ...
  console.log(`- Retrieved ${userPresentations?.length || 0} presentations for user`);
  ...
  if (insertError) {
    ...
    const slideId = Array.isArray(slideData) ? slideData[0]?.id : slideData?.id;
  ...
  const { data: tempPresentation } = await supabase.from('presentations').insert({
  ...
  const tempId = Array.isArray(tempData) ? tempData[0]?.id : tempData?.id;

  console.log(`- Created temporary presentation with id: ${tempId}`);
