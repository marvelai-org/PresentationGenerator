...
jest.mock('../EditableSlide', () => ({
  ...
  default: ({
    ...
    <div data-slide-id={slide.id} data-testid="editable-slide">
      <h3>{slide.title}</h3>
      <button onClick={() => onUpdateTitle('Updated Title')}>Update Title</button>
...
describe('PresentationEditorPage Integration', () => {
  ...
  test('should render the editor with initial slides', () => {
    ...
    expect(screen.getByText('Can We REALLY Stop Climate Change?')).toBeInTheDocument();
  ...
  test('should open and close shape selector', async () => {
    ...
    const shapeTab = screen.getByText(/Shape/i).closest('button') || screen.getByText(/Shape/i);
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('shape-selector-modal')).not.toBeInTheDocument();
    });
  });
  ...
  test('should add a shape to the slide', async () => {
    ...
    const initialContentCount = initialSlide.querySelectorAll('[data-item-id]').length;
    ...
    await waitFor(() => {
      ...
      const newContentCount = updatedSlide.querySelectorAll('[data-item-id]').length;

      expect(newContentCount).toBe(initialContentCount + 1);

      // Find the new shape item
      const shapeItems = updatedSlide.querySelectorAll('[data-item-type="shape"]');
  ...
  test('should add and remove content from a slide', () => {
    ...
    const removeButton = lastContent.querySelector('button:last-of-type') as HTMLButtonElement;
  ...
  test('should update slide title', () => {
    ...
    // Get initial title
    const initialTitle = screen.getByTestId('editable-slide').querySelector('h3')?.textContent;
    ...
    const updatedTitle = screen.getByTestId('editable-slide').querySelector('h3')?.textContent;
  ...
  test('should add a table to the slide', async () => {
    ...
    // Open table selector
    const tableTab = screen.getByText(/Table/i).closest('button') || screen.getByText(/Table/i);
    ...
    await waitFor(() => {
      ...
      // Find the table item
      const tableItems = updatedSlide.querySelectorAll('[data-item-type="table"]');
  ...
  test('should add an embed to the slide', async () => {
    ...
    // Open embed selector
    const embedTab = screen.getByText(/Embed/i).closest('button') || screen.getByText(/Embed/i);
    ...
    await waitFor(() => {
      ...
      // Find the embed item
      const embedItems = updatedSlide.querySelectorAll('[data-item-type="embed"]');
