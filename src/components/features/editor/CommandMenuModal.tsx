...
import { Modal, ModalContent, Button, Tabs, Tab, ScrollShadow, Kbd, tv } from '@heroui/react';
...
export default function CommandMenuModal({
  ...
  return (
    ...
    >
      <ModalContent>
        <Command className={slots.base()} label="Command menu" shouldFilter={false}>
          {showSearch && (
          ...
          {tabs && (
            <>
              ...
              <ScrollShadow className="flex-1 p-4">
                {tabs.map(tab => tab.key === activeTab && <div key={tab.key}>{tab.content}</div>)}
              </ScrollShadow>
            </>
          )}
        </Command>
