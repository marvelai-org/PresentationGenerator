...
import SlideEditor from './core/SlideEditor';
...
export default function EditableSlide({
  ...
  const [hoveredSection, setHoveredSection] = useState<null | 'title' | string>(null);
  const [activeSection, setActiveSection] = useState<null | 'title' | string>(null);
  ...
  // Log the slide data to debug
  console.log(`Rendering slide ${slide.id} with image:`, slide.image);
  ...
  if (slide.gradient) {
    ...
    console.log(`Slide ${slide.id} using gradient:`, slide.gradient);
  } else if (slide.image) {
    // Validate the image URL - now accept both http and https URLs as well as mock URLs
    // For mock URLs, replace with a real placeholder
    if (slide.image.startsWith('http')) {
      console.log(`Using image for slide ${slide.id}: ${slide.image}`);

      // If it's a mock URL (example.com), replace with real placeholder
      if (slide.image.includes('example.com')) {
        // Use a consistent placeholder image service
        const placeholderUrl = `https://picsum.photos/seed/${slide.id}/800/600`;

        console.log(`Replacing mock URL with placeholder: ${placeholderUrl}`);
        backgroundStyles.backgroundImage = `url(${placeholderUrl})`;
      } else {
      ...
      console.warn(`Invalid image URL for slide ${slide.id}: ${slide.image}`);
      // Still try to use whatever URL was provided
      backgroundStyles.backgroundImage = `url(${slide.image})`;
    }
  } else {
    console.log(`Slide ${slide.id} has no image or gradient`);
  }

  // Debug the final background styles
  console.log(`Final background styles for slide ${slide.id}:`, backgroundStyles);
  ...
  const findShapeById = (id: string) => {
    return slide.content.find(item => item.id === id && item.type === 'shape');
  };

  // Find table by id
  const findTableById = (id: string) => {
    return slide.content.find(item => item.id === id && item.type === 'table');
  };

  // Find embed by id
  const findEmbedById = (id: string) => {
    return slide.content.find(item => item.id === id && item.type === 'embed');
  };

  // Find image by id
  const findImageById = (id: string) => {
    return slide.content.find(item => item.id === id && item.type === 'image');
  };
  ...
  const handleDrag = (e: MouseEvent) => {
    ...
    // Try to find shape or image
    const shapeItem = findShapeById(draggingElement);
    const imageItem = findImageById(draggingElement);

    const item = shapeItem || imageItem;
    ...
    } else if (imageItem && onUpdateShape) {
      onUpdateShape(draggingElement, {
        x: newX,
        y: newY,
      });
    }
  ...
  const handleRotateStart = (e: React.MouseEvent | React.KeyboardEvent, itemId: string) => {
    ...
    if ('clientX' in e) {
      ...
      const initialAngle =
        Math.atan2(e.clientY - shapeCenterY, e.clientX - shapeCenterX) * (180 / Math.PI);
  ...
  const updateShapeProperty = (shapeId: string, properties: Partial<SlideContentItem>) => {
    ...
    if (onUpdateShape) {
      ...
      if (shapeElement) {
        if (properties.x !== undefined) shapeElement.style.left = `${properties.x}px`;
        if (properties.y !== undefined) shapeElement.style.top = `${properties.y}px`;
        if (properties.width !== undefined) shapeElement.style.width = `${properties.width}px`;
        if (properties.height !== undefined) shapeElement.style.height = `${properties.height}px`;
  ...
  const renderShape = (item: SlideContentItem) => {
    ...
    const style: React.CSSProperties = {
      ...
      borderWidth: item.style?.borderWidth || (item.style?.borderStyle === 'solid' ? 2 : 0),
      borderStyle: item.style?.borderStyle || 'none',
      color: item.style?.color || 'white',
      transform: item.style?.rotation ? `rotate(${item.style.rotation}deg)` : undefined,
    ...
    if (shapeType.includes('square')) {
      ...
      ShapeElement = (
        <div
          ...
          onMouseDown={e => handleDragStart(e, item.id, item.x || 0, item.y || 0)}
        />
      );
  ...
  // Render image content item
  const renderImage = (item: SlideContentItem) => {
    const x = item.x || 0;
    const y = item.y || 0;
    const width = item.width || 300;
    const height = item.height || 200;
    const rotation = item.style?.rotation || 0;
    const opacity = item.style?.opacity !== undefined ? item.style.opacity : 1;
    const zIndex = item.style?.zIndex || 1;

    // The image URL is stored in the value property
    ...
    return (
      <div
        ...
        onMouseDown={e => handleDragStart(e, item.id, x, y)}
      >
        <img
          alt="Content image"
  ...
  return (
    ...
    >
      {/* Add a semi-transparent overlay when an image is set for better text readability */}
      {slide.image && (
        <div
      ...
      <div className="relative w-full h-full z-10 p-12 flex flex-col">
        {/* Title Section */}
        <div
          ...
          {hoveredSection === 'title' && (
            <div className="absolute right-0 top-1/2 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
              <Tooltip content="Edit title" placement="right">
                <Button isIconOnly className="bg-black/50 text-white" size="sm" variant="flat">
        ...
        <div className="flex-1">
          {slide.content
            ...
            .map(contentItem => (
              <div
                key={contentItem.id}
                className={`relative group mb-4 ${hoveredSection === contentItem.id ? 'bg-white/5' : ''} ${activeSection === contentItem.id ? 'bg-white/10' : ''} rounded-md p-1`}
                role="button"
                tabIndex={0}
                onClick={() => handleSectionClick(contentItem.id)}
                onKeyDown={e => {
                  if (e.key === 'Enter' || e.key === ' ') {
                    handleSectionClick(contentItem.id);
                  }
                }}
                onMouseEnter={() => setHoveredSection(contentItem.id)}
                onMouseLeave={() => setHoveredSection(null)}
              >
                {activeSection === contentItem.id ? (
                  <SlideEditor
                    content={contentItem.value}
                    placeholder="Add content..."
                    onUpdate={newValue => onUpdateContent(contentItem.id, newValue)}
                  />
                ) : (
                  <p className="text-xl text-white p-1">{contentItem.value}</p>
                )}

                {hoveredSection === contentItem.id && activeSection !== contentItem.id && (
        ...
        {/* Render shape content separately */}
        {slide.content
          .filter(
            item =>
              item.type === 'shape' ||
        ...
        {isTitleSlide && slide.author && (
          <div className="mt-auto flex items-center">
            ...
            <div className="ml-4">
              ...
              {slide.editedTime && (
                <div className="text-gray-400 text-sm">Last edited {slide.editedTime}</div>
              )}
            </div>
          </div>
        )}
