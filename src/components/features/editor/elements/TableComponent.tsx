  ...
  const addRow = useCallback(
    (position: 'before' | 'after') => {
      ...
      try {
        ...
        const newRowIndex = position === 'before' ? selectedRow : selectedRow + 1;
  ...
  const addColumn = useCallback(
    (position: 'before' | 'after') => {
      ...
      try {
        ...
        const newColIndex = position === 'before' ? selectedCol : selectedCol + 1;
      ...
      >
            ...
            {Array.from({ length: tableData.rows }).map((_, rowIndex) => (
              <tr key={rowIndex}>
                {Array.from({ length: tableData.columns }).map((_, colIndex) => {
                  ...
                  if (!cell) {
                    return (
                      <td key={cellId} className="border border-red-500 p-2 bg-red-100" id={cellId}>
        ...
        {selected && isEditing && (
          <div className="absolute -top-10 right-0 flex items-center gap-1">
            <Dropdown>
              <DropdownTrigger>
                <Button className="bg-gray-800 text-white" size="sm" variant="flat">
                  <Icon className="mr-1" icon="material-symbols:table-rows" width={16} />
                  Rows
                </Button>
              </DropdownTrigger>
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button className="bg-gray-800 text-white" size="sm" variant="flat">
                  <Icon className="mr-1" icon="material-symbols:table-columns" width={16} />
                  Columns
                </Button>
              </DropdownTrigger>
