...
export default function ShapeSelector({
  ...
  const renderShapeItem = (shape: any, filled: boolean = true) => {
    ...
    return (
      ...
      >
        {shape.icon ? (
          <Icon
            className={`${fillClass} shape-preview`}
            height={40}
            icon={shape.icon}
            width={40}
          />
    ...
    return (
      ...
      >
        <div className="w-full flex items-center justify-center relative">
          ...
          {line.type === "dashed" && !line.icon && (
            <div className={styles.lineDashed} />
          )}
          {line.type === "dotted" && !line.icon && (
            <div className={styles.lineDotted} />
          )}
  ...
  const renderLinesTab = () => (
    <div>
      ...
      <div className="grid grid-cols-6 gap-3 mb-8">
        {lineTypes
          .filter((line) => line.type === "solid")
          .map((line) => renderLineItem(line))}
      </div>
      <div className="grid grid-cols-6 gap-3 mb-8">
        {lineTypes
          .filter((line) => line.type === "dashed")
          .map((line) => renderLineItem(line))}
      </div>
      <div className="grid grid-cols-6 gap-3">
        {lineTypes
          .filter((line) => line.type === "dotted")
          .map((line) => renderLineItem(line))}
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {buttonShapes.map((shape) => (
          ...
          >
            ...
            {shape.id === "ribbon" && (
              <div className="w-12 h-12 transform rotate-45 bg-content3" />
            )}
          </div>
        ))}
      </div>
        ...
        {processShapes.map((shape) => (
          ...
          >
            {shape.id === "process-start" && (
              <div
                className={`bg-content3 w-20 h-16 ${styles.clipArrowRight}`}
              />
            )}
      ...
      <div className="grid grid-cols-4 gap-3">
        {processShapes.map((shape) => (
          ...
          >
            {shape.id === "process-start" && (
              <div
                className={`border-2 border-default-400 w-20 h-16 ${styles.clipArrowRight}`}
              />
            )}
            {shape.id === "process-arrow" && (
              <div
            ...
            {shape.id === "process-hexagon" && (
              <div
                className={`border-2 border-default-400 w-20 h-16 ${styles.clipHexagon}`}
              />
            )}
            {shape.id === "process-diamond" && (
              <div
                className={`border-2 border-default-400 w-20 h-16 ${styles.clipDiamond}`}
              />
            )}
          </div>
        ))}
