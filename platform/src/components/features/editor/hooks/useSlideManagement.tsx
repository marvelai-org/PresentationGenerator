...
import { templateToSlide } from '../adapters/TemplateToSlideAdapter';
...
export type TemplateType =
  ...
  | string; // Add string to allow any template ID from registry

// Define interface for slide templates
...
export default function useSlideManagement(
  ...
  // Update currentSlideIndex if slides change and current index is out of bounds
  useEffect(() => {
  ...
  const addSlideFromTemplate = useCallback(
    (templateType: TemplateType) => {
      // First try to create a slide from the new template system
      const newSlide = templateToSlide(templateType);
      ...
      // Fallback to legacy templates if not found in new system
      ...
      const updatedSlides = [...slides, legacySlide];
  ...
  const duplicateSlide = useCallback(
    (index: number) => {
      ...
      const newSlide: Slide = {
        ...
        content: JSON.parse(JSON.stringify(slideToClone.content)), // Deep copy of content
      };
  ...
  // Move slide up (towards start of deck)
  ...
  // Move slide down (towards end of deck)
    ...
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id) {
        ...
        if (activeIndex !== -1 && overIndex !== -1) {
          const updatedSlides = [...slides];
          const [removedSlide] = updatedSlides.splice(activeIndex, 1);
          updatedSlides.splice(overIndex, 0, removedSlide);

          setSlides(updatedSlides);

          // Update current slide index if the current slide was moved
          if (activeIndex === currentSlideIndex) {
            setCurrentSlideIndex(overIndex);
          }
          // Or if slides were inserted before the current slide
          else if (
            (activeIndex < currentSlideIndex && overIndex >= currentSlideIndex) ||
            (activeIndex > currentSlideIndex && overIndex <= currentSlideIndex)
          ) {
            // Adjust the index based on direction of move
            const newIndex =
              activeIndex < currentSlideIndex ? currentSlideIndex - 1 : currentSlideIndex + 1;
  ...
  const updateSlideContent = useCallback(
    (slideId: number, contentId: string, value: string) => {
      ...
      const contentIndex = slides[slideIndex].content.findIndex(item => item.id === contentId);
      if (contentIndex === -1) return;

      const updatedSlides = [...slides];
      updatedSlides[slideIndex].content[contentIndex].value = value;
  ...
  const updateSlideTitle = useCallback(
    (slideId: number, title: string) => {
      ...
      const updatedSlides = [...slides];
      updatedSlides[slideIndex].title = title;
  ...
  const removeContent = useCallback(
    (slideId: number, contentId: string) => {
      ...
      const updatedSlides = [...slides];
      updatedSlides[slideIndex].content = updatedSlides[slideIndex].content.filter(
  ...
  const handleAddContent = useCallback(
    (slideId: number) => {
      ...
      const newContent: SlideContentItem = {
        id: `content-${Date.now()}`,
        type: 'text',
        value: 'New text',
        x: 100,
      ...
      const updatedSlides = [...slides];
      updatedSlides[slideIndex].content.push(newContent);
