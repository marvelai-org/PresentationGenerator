...
import { generateSlides as apiGenerateSlides, generateImages as apiGenerateImages } from '@/services/aiService';
...
async function generateOutline(
  ...
  try {
    console.log('Generating outline with params:', {
    ...
    if (response.status === 504) {
      throw new Error('Gateway timeout: The AI service took too long to respond. Please try again later.');
    } else if (response.status === 502 || response.status === 503) {
      throw new Error('AI service unavailable. Please check if the service is running and try again.');
    ...
    console.log('Successfully generated outline slides:', data.slides?.length || 0);
    ...
    if (error instanceof Error) {
      ...
      throw new Error('Unknown error generating outline');
    }
  }
}
...
export default function OutlinePage() {
  ...
  // Move fetchOutline outside of the useEffect
  const fetchOutline = async () => {
    try {
      setError(null); // Clear any previous errors
      setIsLoading(true);
      
      console.log('Fetching outline with params:', {
        prompt, slideCountParam, templateStyle, language,
        theme: selectedTheme.id, density: textDensity, imageSource, model: aiModel
      });
      
      const numSlides = parseInt(slideCountParam.split(' ')[0], 10);
      
      // Check if AI backend is reachable first
      try {
        const healthCheck = await fetch('/api/outline/test-connection');
        if (!healthCheck.ok) {
          throw new Error('AI service is not available. Please ensure the service is running.');
        }
      } catch (healthError) {
        console.error('Health check failed:', healthError);
        setError('Could not connect to AI service. Please check if the service is running.');
        setIsLoading(false);
        return;
      }
      ...
      // If we get here, outline generation was successful
      // Extract slide titles for content generation
      const slideTitles = outlineData.map(slide => slide.title);
      
      // Generate detailed slide content
      console.log("Generating slide content...");
      try {
        const slideContentData = await apiGenerateSlides({
          slides_titles: slideTitles,
          topic: prompt,
          instructional_level: textDensity === 'Brief' ? 'beginner' : 
                             textDensity === 'Detailed' ? 'advanced' : 'intermediate',
          lang: language.substring(0, 2).toLowerCase()
        });
        
        console.log("Slide content generated:", slideContentData);
        
        // Now generate images for the slides
        console.log("Generating images for slides...");
        try {
          const slideImagesData = await apiGenerateImages({
            slides: slideContentData
          });
          
          console.log("Images generated:", slideImagesData);
          
          // Use the enriched data with content and images
          if (slideImagesData && slideImagesData.slides && Array.isArray(slideImagesData.slides)) {
            const formattedSlides: SlideContent[] = slideImagesData.slides.map((slide, index) => {
              // Extract bullets from the slide content
              let bulletPoints = [];
              if (typeof slide.content === 'string') {
                ...
                bulletPoints = slide.content;
              } else if (typeof slide.content === 'object' && slide.content) {
                // Object with various content sections
                bulletPoints = Object.values(slide.content).filter(val => 
                  typeof val === 'string' && val.trim() !== ''
                );
              }
              
              // Format as HTML bullets
              const bulletPointsHtml = bulletPoints.map(bullet => `<li>${bullet}</li>`).join('');
              const content = `<h2>${slide.title}</h2><ul>${bulletPointsHtml}</ul>`;
          ...
          // Fall back to just using the slide content without images
          const formattedSlides: SlideContent[] = slideContentData.map((slide, index) => {
            // Extract bullets from the slide content
            let bulletPoints = [];
            if (typeof slide.content === 'string') {
              bulletPoints = [slide.content];
        ...
        console.error("Error generating slide content:", contentError);
        
        // Fall back to the basic outline with placeholder bullets
  ...
  // Replace refreshPrompt implementation to use the fetchOutline function
  const refreshPrompt = () => {
    setIsLoading(true);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-4 apple-dark">
      ...
      <style dangerouslySetInnerHTML={highlightAnimationStyle} />
